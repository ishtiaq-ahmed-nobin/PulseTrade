...
export default function AdminLogin() {
    const { adminLogin, saveSession } = useAuth()
    ...
    async function handleSubmit(e) {
        ...
        try {
            ...
            else setError(extractErrors(err, 'Unable to sign in to the admin panel.'))
        } finally {
            setLoading(false)
        }
    }
    ...
    return (
        <div className="space-y-6">
            <div className="text-center">
                <span className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-xl bg-brand-900 text-sky-400">
                    <ShieldCheck size={24} />
                </span>
                <h1 className="text-2xl font-bold text-brand-900 dark:text-white">Admin Sign In</h1>
                <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Restricted area — authorized staff only</p>
            </div>
