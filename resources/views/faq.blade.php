...
<x-storefront-layout :title="'FAQ — PulseTrade'">
    ...
    <div class="bg-navy-950 text-white py-14">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            ...
            <p class="text-ivory/60 mt-2 text-sm max-w-lg mx-auto">Everything you need to know about ordering, shipping, returns, and your account. Can't find what you're looking for? <a href="{{ url('/contact') }}" class="text-pulse-400 hover:text-pulse-300 font-medium">Contact us</a>.</p>
        </div>
    </div>
                ...
                <div class="space-y-3" x-data="{ open: null }">
                    @foreach ($faqs as $i => $faq)
                        <div class="rounded-xl border border-navy-100 bg-white overflow-hidden">
                            <button @click="open = open === '{{ $group }}-{{ $i }}' ? null : '{{ $group }}-{{ $i }}'"
                                class="w-full flex items-center justify-between px-6 py-4 text-left">
                                <span class="text-sm font-semibold text-navy-900 pr-4">{{ $faq['q'] }}</span>
                                <svg class="w-4 h-4 text-navy-700/50 shrink-0 transition-transform" :class="open === '{{ $group }}-{{ $i }}' ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                                </svg>
                            </button>
                            <div x-show="open === '{{ $group }}-{{ $i }}'" x-collapse x-cloak class="px-6 pb-4">
                                <p class="text-sm text-navy-700/70 leading-relaxed">{{ $faq['a'] }}</p>
                            </div>
                        </div>
                    @endforeach
                </div>
    ...
    <section class="bg-ivory py-16">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            ...
            <div class="mt-8 flex flex-col sm:flex-row gap-4 justify-center">
                <a href="{{ url('/contact') }}" class="inline-flex items-center justify-center px-7 py-3.5 rounded-full bg-pulse-500 hover:bg-pulse-400 text-white font-semibold text-sm transition-colors">
                    Contact Support
                </a>
