...
<x-storefront-layout :title="'Blog — PulseTrade'">
    ...
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <a href="{{ url('/blog/' . $posts[0]['slug']) }}" class="grid lg:grid-cols-2 gap-8 items-center group rounded-3xl border border-navy-100 overflow-hidden hover:shadow-lg hover:shadow-navy-900/5 transition-shadow">
            <div class="aspect-[4/3] lg:aspect-square bg-gradient-to-br {{ $posts[0]['gradient'] }} flex items-center justify-center">
            ...
            <div class="p-8 lg:p-10">
                <div class="flex items-center gap-3 mb-4">
                    <span class="text-xs font-bold tracking-wider uppercase text-pulse-500">{{ $posts[0]['category'] }}</span>
                    <span class="text-xs text-navy-700/40">•</span>
                    <span class="text-xs text-navy-700/50">{{ $posts[0]['date'] }}</span>
                    <span class="text-xs text-navy-700/40">•</span>
                    <span class="text-xs text-navy-700/50">{{ $posts[0]['readTime'] }}</span>
                </div>
                <h2 class="font-display text-2xl sm:text-3xl font-bold text-navy-900 group-hover:text-pulse-500 transition-colors">{{ $posts[0]['title'] }}</h2>
                <p class="mt-4 text-navy-700/70 leading-relaxed text-sm">{{ $posts[0]['excerpt'] }}</p>
    ...
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20">
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach ($posts->slice(1) as $post)
                <a href="{{ url('/blog/' . $post['slug']) }}" class="group rounded-2xl border border-navy-100 bg-white overflow-hidden hover:shadow-lg hover:shadow-navy-900/5 transition-shadow">
                    ...
                    <div class="p-5">
                        <div class="flex items-center gap-2 mb-3">
                            <span class="text-[10px] font-bold tracking-wider uppercase text-pulse-500">{{ $post['category'] }}</span>
                            <span class="text-[10px] text-navy-700/40">•</span>
                            <span class="text-[10px] text-navy-700/50">{{ $post['date'] }}</span>
                        </div>
                        <h3 class="font-display font-semibold text-navy-900 leading-snug group-hover:text-pulse-500 transition-colors">{{ $post['title'] }}</h3>
                        <p class="text-xs text-navy-700/60 mt-2 leading-relaxed line-clamp-2">{{ $post['excerpt'] }}</p>
                        <div class="flex items-center justify-between mt-4">
                            <span class="text-xs text-navy-700/50">{{ $post['readTime'] }}</span>
