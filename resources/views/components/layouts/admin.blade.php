...
<head>
    ...
    <title>{{ $title ?? 'Admin' }} — PulseTrade</title>
    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
        ...
        <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-1">
            ...
            <a href="{{ route('admin.dashboard') }}"
               class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors {{ $isActiveDashboard ? 'bg-pulse-500/15 text-pulse-400' : 'text-white/60 hover:text-white hover:bg-white/5' }}">
            ...
            @foreach ($navGroups as $group)
                ...
                <div class="flex items-center justify-between gap-2 px-3 pt-4 pb-1">
                    <span class="text-[11px] font-semibold uppercase tracking-wider {{ $groupActive ? 'text-pulse-400' : 'text-white/30' }}">{{ $group['label'] }}</span>
                    @if (isset($group['badge']) && $group['badge'] > 0)
                        <span class="inline-flex items-center justify-center min-w-[20px] h-5 px-1.5 text-[10px] font-bold rounded-full bg-red-500 text-white leading-none">{{ $group['badge'] }}</span>
                    @endif
                </div>
                ...
                <div class="space-y-0.5 pl-1">
                    @foreach ($group['items'] as $item)
                        ...
                        <a href="{{ route($item['route']) }}"
                           class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors {{ $isActive ? 'bg-pulse-500/15 text-pulse-400' : 'text-white/60 hover:text-white hover:bg-white/5' }}">
                            <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">{!! $item['icon'] !!}</svg>
                            <span class="flex-1 text-left">{{ $item['label'] }}</span>
                            @if (isset($item['badge']) && $item['badge'] > 0)
                                <span class="inline-flex items-center justify-center min-w-[20px] h-5 px-1.5 text-[10px] font-bold rounded-full {{ $item['badge_color'] ?? 'bg-red-500 text-white' }}">{{ $item['badge'] }}</span>
                            @endif
                        </a>
                    @endforeach
                </div>
        ...
        <div class="border-t border-white/10 p-3 shrink-0">
            <a href="{{ url('/') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-white/60 hover:text-white hover:bg-white/5 transition-colors">
