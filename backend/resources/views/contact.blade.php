...
<x-storefront-layout :title="'Contact Us — PulseTrade'">
    ...
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        <div class="grid lg:grid-cols-[1fr_1.4fr] gap-12">
                ...
                @foreach ($contactCards as $card)
                    <div class="flex items-start gap-4 rounded-2xl border border-navy-100 p-5 hover:shadow-lg hover:shadow-navy-900/5 transition-shadow">
                        <div class="w-11 h-11 rounded-full bg-pulse-50 flex items-center justify-center shrink-0">
                            <svg class="w-5 h-5 text-pulse-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">{!! $card['icon'] !!}</svg>
                        </div>
                        <div>
                            <p class="font-display font-semibold text-navy-900">{{ $card['title'] }}</p>
                            <p class="text-sm text-navy-700 mt-0.5">{{ $card['detail'] }}</p>
                            <p class="text-xs text-navy-700/50 mt-1">{{ $card['sub'] }}</p>
                        </div>
                    </div>
                @endforeach
            ...
            <div class="rounded-2xl border border-navy-100 bg-white p-8 sm:p-10">
                ...
                <form method="POST" action="{{ url('/contact') }}" class="mt-8 space-y-5">
                    @csrf
                ...
                @foreach ($faqs as $i => $faq)
                    <div class="rounded-xl border border-navy-100 bg-white overflow-hidden">
                        <button @click="open = open === {{ $i }} ? null : {{ $i }}"
                            ...
                            <span class="text-sm font-semibold text-navy-900">{{ $faq['q'] }}</span>
                            <svg class="w-4 h-4 text-navy-700/50 shrink-0 transition-transform" :class="open === {{ $i }} ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
